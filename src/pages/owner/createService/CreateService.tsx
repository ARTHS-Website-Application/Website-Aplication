...
import { itemProduct, item, selectorProduct } from '@/types/actions/product';
...
const CreateService = () => {
  ...
  const [checkedProduct, setCheckedProduct] = useState<item<string,number>[]>([]);
  ...
  useEffect(() => {
    if (productInfor.data)
      setDataProduct(productInfor.data)
  }, [productInfor.data])
  ...
  const handleCreateService = () => {
    ...
    if(nameProduct && priceProduct && addProduct && descriptionProduct && images){
            ...
            {checkedProduct?.length>0 && (
              <div className='overflow-y-scroll h-[10vh] px-3 py-4 space-y-3 mb-3'>
                ...
                <div key={index} className='flex justify-between items-center'>
                  <div className='flex items-center space-x-2'>
                    <img src={item?.images[0].imageUrl} className='w-11 h-11'/>
                  ...
                  {itemSearch?.length > 0
                    ? itemSearch?.map((item, index) => (
                      <div className='flex space-x-3 space-y-3 items-center' key={index}>
                        ...
                        <img src={item?.images[0].imageUrl} className='w-14 h-14' />
